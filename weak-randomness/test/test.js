...
describe("Weak Randomness", function () {
  ...
  describe("Lottery", function () {
    describe.skip("With bets open", function () {
      it("Should allow a user to place bet", async function () {
        await this.lottery.placeBet(5, { value: ethers.utils.parseEther("10") });
        expect(await this.lottery.bets(deployer.address)).to.eq(5);
      });
      it("Should revert if a user places more than 1 bet", async function () {
        await this.lottery.placeBet(5, { value: ethers.utils.parseEther("10") });
        await expect(this.lottery.placeBet(50, { value: ethers.utils.parseEther("10") })).to.be.revertedWith("Only 1 bet per player");
      });
      it("Should revert if bet value is not 10 ethers", async function () {
        await expect(this.lottery.placeBet(100, { value: ethers.utils.parseEther("5") })).to.be.revertedWith("Bet cost: 10 ether");
      });
      it("Should revert if bet number is less than 1", async function () {
    ...
    describe("Attack", function () {
      it.skip("A miner could tamper the results", async function () {
        ...
        await ethers.provider.send("evm_setNextBlockTimestamp", [1641482343]);
